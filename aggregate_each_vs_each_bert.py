...
llms_dct = {'llama': {0: 'bert', 1: 'bert_1', 2: 'bert_2'}, 'gpt': {0: 'results_bert', 1: 'results_bert_1', 2: 'results_bert_2'}}
...
def get_each_vs_each(class_methods, llm_methods):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_bert/'+llm_methods+'_vs_'+class_methods+'_'+llm+'.csv', index=False)
...
def get_each_vs_clean(method, is_llm):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_bert/'+method+'_vs_clean_'+llm+'.csv', index=False)
