...
def get_samples_per_seed(df_orig, df_aug, no_seeds, no_samples):
    ...
    return tmp_data.sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()[['text', 'label']], df_orig_sub.rename(columns={'seed': 'text'})
...
for no_seed_samples in lst_no_seeds_samples:
    for method in methods: 
        ...
        train_file = 'yelp/data/yelp_'+method+'.csv'
        ...
        for no_coll_samples in lst_no_coll_samples:
            ...
            res_file = 'yelp/results/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/yelp_'+method+'_seeds_'+str(no_seed_samples)+'_coll_'+str(no_coll_samples)+'_class.csv'
