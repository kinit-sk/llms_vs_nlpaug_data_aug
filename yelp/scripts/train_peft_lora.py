...
def get_samples_per_seed(df_orig, df_aug, no_seeds, no_samples):
    ...
    if no_seeds != 100:
        ...
        df_orig_sub = df_orig.rename(columns={'text': 'seed'})
...
df_orig = pd.read_csv('yelp/data/yelp_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('yelp/data/test_yelp.csv').drop_duplicates().dropna()
...
for no_seed_samples in lst_no_seeds_samples:
    for method in methods: 
        ...
        train_file = 'yelp/data/yelp_paraphrases_train_'+method+'_w_tryout.csv'
        ...
        for no_coll_samples in lst_no_coll_samples:
            ...
            for repeat_no in range(0, args.repeat):
                ...
                model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=5) #classifier_dropout
            ...
            res_file = 'yelp/results/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/yelp_'+method+'_seeds_'+str(no_seed_samples)+'_coll_'+str(no_coll_samples)+'_llm.csv'
