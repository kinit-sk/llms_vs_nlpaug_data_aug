...
def get_samples_per_seed(df_orig, df_aug, no_seeds, no_samples):
    ...
    try:
        ...
        df_orig_sub = df_orig.groupby('label', group_keys=False).apply(lambda x: x.sample(no_seeds, replace=True, random_state=args.seed))
    df_orig_sub = df_orig_sub.rename(columns={'text': 'seed'})
    
    df_orig_sub['seed'] = df_orig_sub['seed'].astype(str)
    df_aug['seed'] = df_aug['seed'].astype(str)
    
    df_data = df_orig_sub.merge(df_aug, how='inner', on=['seed']).rename(columns={'label_x':'label'})[['text', 'label', 'seed']]
    
    try:
        tmp_data = df_data.groupby('seed', group_keys=False).apply(lambda x: x.sample(no_samples, random_state=args.seed))
    except:
        tmp_data = df_data.groupby('seed', group_keys=False).apply(lambda x: x.sample(no_samples, replace=True, random_state=args.seed))
        
    return tmp_data.sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates()[['text', 'label']], df_orig_sub.rename(columns={'seed': 'text'})

df_orig = pd.read_csv('ag_news/data/news_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('ag_news/data/news_seeds_test.csv').drop_duplicates().dropna()
...
methods = ['clean'] #['w2v', 'cont',
lst_no_seeds_samples = [5, 10, 20, 30, 40, 50, 100]
lst_no_coll_samples = [1, 2, 5, 10, 15]

dct_seeds_to_inc = {42: '', 1: '_1', 2: '_2'}

#dct_model_to_dir = {'FacebookAI/roberta-base': 'results_orig', 'google-bert/bert-base-uncased':'results_bert', 'distilbert/distilbert-base-uncased':'results_distilbert'}
dct_model_to_dir = {'FacebookAI/roberta-base': 'roberta_lora', 'google-bert/bert-base-uncased':'bert_lora', 'distilbert/distilbert-base-uncased':'distilbert_lora'}
...
for method in methods: 
    ...
    for no_seed_samples in lst_no_seeds_samples:
        ...
        for repeat_no in range(0, args.repeat):
            ...
            #     model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=4, dropout= 0.2)
            # else:
            #     model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=4, classifier_dropout= 0.2)
            model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=4)
            if args.base_model_type == 'distilbert/distilbert-base-uncased':
                peft_config = LoraConfig(task_type="SEQ_CLS", r=16, lora_alpha=16, lora_dropout=0.1, target_modules=["q_lin", "k_lin","v_lin"])
            else:
                peft_config = LoraConfig(task_type="SEQ_CLS", r=16, lora_alpha=16, lora_dropout=0.1)
            model = get_peft_model(model, peft_config)
            
            df_orig_sub = df_orig.groupby('label', group_keys=False).apply(lambda x: x.sample(no_seed_samples, random_state=args.seed))
            ds_train = prepare_data(df_orig_sub)
        ...
        res_file = 'ag_news/results/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/news_'+method+'_seeds_'+str(no_seed_samples)+'.csv'
