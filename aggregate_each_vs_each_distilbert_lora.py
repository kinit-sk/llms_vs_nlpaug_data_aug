...
llms_dct = {'llama': {0: 'distilbert_lora', 1: 'distilbert_lora_1', 2: 'distilbert_lora_2'}, 'gpt': {0: 'results_distilbert_lora', 1: 'results_distilbert_lora_1', 2: 'results_distilbert_lora_2'}}
...
def get_each_vs_each(class_methods, llm_methods):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_distilbert_lora/'+llm_methods+'_vs_'+class_methods+'_'+llm+'.csv', index=False)
...
def get_each_vs_clean(method, is_llm):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_distilbert_lora/'+method+'_vs_clean_'+llm+'.csv', index=False)
