...
df_orig = pd.read_csv('news_category/data/news_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('news_category/test_news.csv').drop_duplicates().dropna()
...
methods = ['para', 'ins', 'swap']
lst_no_seeds_samples = [5, 10, 20, 30, 40,50, 100]
lst_no_coll_samples = [1, 2, 5, 10, 15]

dct_seeds_to_inc = {42: '', 1: '_1', 2: '_2'}

#setup for LLama3, exact same is used for GPT--3.5 data: you need to change this dict and the train files loaded though
...
for no_seed_samples in lst_no_seeds_samples:
    for method in methods: 
        ...
        train_file = 'news_category/data/news_paraphrases_train_'+method+'_w_tryout.csv'
        df_aug_full = pd.read_csv(train_file)
        for no_coll_samples in lst_no_coll_samples:
            ...
            for repeat_no in range(0, args.repeat):
                ...
                model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=6)
                ...
                df_aug, df_orig_sub = get_samples_per_seed(df_orig, df_aug_full, no_seed_samples, no_coll_samples)
                dfs = [df_orig_sub, df_aug]
                print(str(len(pd.concat(dfs))))
            ...
            res_file = 'news_category/results/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/news_'+method+'_seeds_'+str(no_seed_samples)+'_coll_'+str(no_coll_samples)+'_llm.csv'
