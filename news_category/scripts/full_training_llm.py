...
df_orig = pd.read_csv('news_category/aug_data/csv/news_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
        ...
        for no_coll_samples in lst_no_coll_samples:
            ...
            for repeat_no in range(0, args.repeat):
                ...
                if args.base_model_type == 'distilbert/distilbert-base-uncased':
                    model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=6, dropout= 0.2)
                else:
                    model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=6, classifier_dropout= 0.2)
