...
llms_dct = {'llama': {0: 'roberta_lora', 1: 'roberta_lora_1', 2: 'roberta_lora_2'}, 'gpt': {0: 'results_roberta_lora', 1: 'results_roberta_lora_1', 2: 'results_roberta_lora_2'}}
...
def get_each_vs_each(class_methods, llm_methods):
    for llm in llms_dct.keys():
                ...
                for seed in seeds_to_iter:
                    ...
                    for col in cols:
                        for method in [class_methods]:
                            ...
                            #print(dataset_dir+'/'+llms_dct['llama'][rand_seed]+'/'+datasets[dataset_dir]+'_'+method+'_seeds_'+str(seed)+'_coll_'+str(col)+'_class'+llms_iter_dct['llama'][rand_seed]+'.csv')
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_roberta_lora/'+llm_methods+'_vs_'+class_methods+'_'+llm+'.csv', index=False)
...
def get_each_vs_clean(method, is_llm):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_roberta_lora/'+method+'_vs_clean_'+llm+'.csv', index=False)
