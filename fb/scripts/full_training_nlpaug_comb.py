...
df_orig = pd.read_csv('fb_under/data/fb_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('fb_under/data/fb_test_fix.csv').drop_duplicates().dropna()
...
train_file0 = 'fb_under/data/fb_'+methods[0]+'.csv'
train_file1 = 'fb_under/data/fb_'+methods[1]+'.csv'
...
for no_seed_samples in lst_no_seeds_samples:
    for no_coll_samples in lst_no_coll_samples:
        ...
        res_file = 'fb_under/results/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/fb_comb_seeds_'+str(no_seed_samples)+'_coll_'+str(no_coll_samples)+'_class.csv'
