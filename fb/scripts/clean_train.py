...
df_orig = pd.read_csv('fb_under/aug_data/csv/fb_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('fb_under/aug_data/csv/fb_test_fix.csv').drop_duplicates().dropna()
...
methods = ['clean'] #['w2v', 'cont', 'cont_ins', 'back', 'rand', 'spell']
lst_no_seeds_samples = [5, 10, 20, 30, 40, 50]
...
dct_model_to_dir = {'FacebookAI/roberta-base': 'bert_lora', 'google-bert/bert-base-uncased':'bert_true_lora', 'distilbert/distilbert-base-uncased':'distilbert_lora'}
...
for method in methods: 
    ...
    for no_seed_samples in lst_no_seeds_samples:
        ...
        for repeat_no in range(0, args.repeat):
            ...
            #     model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=7, dropout= 0.2)
            # else:
            #     model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=7, classifier_dropout= 0.2)
                
            model = AutoModelForSequenceClassification.from_pretrained(args.base_model_type, num_labels=7)
            if args.base_model_type == 'distilbert/distilbert-base-uncased':
        ...
        res_file = 'fb_under/aug_data/'+dct_model_to_dir[args.base_model_type]+''+dct_seeds_to_inc[args.seed]+'/fb_'+method+'_seeds_'+str(no_seed_samples)+'.csv'
