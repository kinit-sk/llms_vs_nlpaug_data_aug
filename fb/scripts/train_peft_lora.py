...
df_orig = pd.read_csv('fb/data/fb_seeds_train.csv').sample(frac=1, random_state=args.seed).reset_index(drop=True).drop_duplicates().dropna()
df_orig['text'] = df_orig['text'].str.lower()
#df_orig['label'] = (~df_orig.label.astype(bool)).astype(int)
ds_train_orig = prepare_data(df_orig)
df_test_orig = pd.read_csv('fb/data/fb_test_fix.csv').drop_duplicates().dropna()
...
methods = ['para', 'ins', 'swap'] #['para', 'swap', 'ins', 'rand',
...
for method in methods: 
    ...
    train_file = 'fb/data/fb_paraphrases_train_'+method+'_w_tryout.csv'
    ...
    for no_seed_samples in lst_no_seeds_samples:
        for no_coll_samples in lst_no_coll_samples:
            ...
            res_file = 'fb/results/distilbert_lora'+dct_seeds_to_inc[args.seed]+'/fb_'+method+'_seeds_'+str(no_seed_samples)+'_coll_'+str(no_coll_samples)+'_llm.csv'
