...
llms_dct = {'llama': {0: 'bert_lora', 1: 'bert_lora_1', 2: 'bert_lora_2'}, 'gpt': {0: 'results_bert_lora', 1: 'results_bert_lora_1', 2: 'results_bert_lora_2'}}
llms_iter_dct = {'gpt': {0: '', 1: '', 2: ''}, 'llama': {0: '', 1: '', 2: ''}}
    ...
    for llm in llms_dct.keys():
        ...
        for rand_seed in range(0,3):
            for dataset_dir in datasets.keys():
                ...
                for seed in seeds_to_iter:
                    ...
                    for col in cols:
                        ...
                        for method in [llm_methods]:
                            #print(dataset_dir+'/'+llms_dct[llm][rand_seed]+'/'+datasets[dataset_dir]+'_'+method+'_seeds_'+str(seed)+'_coll_'+str(col)+'_llm'+llms_iter_dct[llm][rand_seed]+'.csv')
                ...
                #print(dataset_dir)
                #print(seed)
                    #print(best_llm_method_val.mean())
                    #print(best_llm_method_name)
                    #print(best_llm_method_col)
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_bert_lora/'+llm_methods+'_vs_'+class_methods+'_'+llm+'.csv', index=False)
...
def get_each_vs_clean(method, is_llm):
    for llm in llms_dct.keys():
        ...
        pd.DataFrame.from_dict(dct_df_final).to_csv('each_vs_each_bert_lora/'+method+'_vs_clean_'+llm+'.csv', index=False)
